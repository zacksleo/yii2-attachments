...
class Gallery extends ActiveRecord
{
    ...
    {
        return [
            'id' => Module::t('attachments', 'ID'),
            'name' => Module::t('attachments', 'Name'),
            'status' => Module::t('attachments', 'Status'),
            'created_at' => Module::t('attachments', 'Created'),
            'updated_at' => Module::t('attachments', 'Updated'),
        ];
    }
    ...
    private function updatePositions()
    {
        ...
        $sql = 'UPDATE {{%attachment}} SET position = CASE id' . $when . ' END WHERE id IN (' . implode(', ',
                $where) . ')';
    ...
    {
        return $this->hasMany(Attachment::className(), ['gallery_id' => 'id'])
            ->orderBy([
                'position' => SORT_ASC
            ]);
    }
